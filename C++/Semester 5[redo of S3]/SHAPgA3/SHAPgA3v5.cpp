...
{	char choice[100]; // An array we use to get the choice from our user
	...
	{	printf("Choice: ");
		gets(choice); // Storing the input into our variable
...
a final sorted array one item at a time. It's much less
efficient than other more advanced algorithms.
It has very simple implementation, it's very adaptive
and it's also very stable. 

It simply takes a value(or set of values) and finds their spot in the array
by checking against every value until it finds a suitable location
...
{	printf("Insetion is running\n");
	...
	do
	{   strcpy(y, name[k]); // copy name into temp location
		weightQ = weight[k]; // copy weight into temp location
		i = k - 1;
		found = false;
		while (i >= 0 && !found)
		{	if (strcmp(y, name[i]) < 0) // y is in the incorrect position
		...
		strcpy(name[i + 1], y); // move y into correct posotion
		weight[i + 1] = weightQ; // move weight into correct positon
		k = k + 1;
	} while (k <= lsize - 1);
	system("pause");
}
...
Selection sort is a sorting algorithm, specifically
an in-place comparison sort. It is inefficient on large lists
and performs worse than the similar insertion sort. But it has certain 
performance advantages over more complex sorting algorithms in certain
situations

This sorting algorithm tries to find the smallest element
in an unsorted array and than places it in the sorted array 
and then increment j(the sublist boundary)
...
{	printf("Selection sort is running\n");
	...
	{	strcpy(big, name[0]); // Initially storing the first subscript / name into the biggest spot
		...
		i = i - 1; 
	} while (i > 0); // is i still bigger than 0?
	system("pause");
}
...
This is also an in-place comparison sort. 
it can be seen as either a generalization
of sorting by exchange(similar to bubble sort).

The algorithm starts by sorting the elements far apart
from each other and progressively reduces the gap between
...
{	printf("Shell sort running\n");
	...
	do
	{	gap = gaplist[i];
		j = gap; // the current gap we're working with
		do
		{	strcpy(y, name[j]);
			weightQ = wght[j];
			k = j - gap;
			found = false;
			while (k >= 0 && !found) // keep going until we find the position or rip through the whole array
			{	if (strcmp(y, name[k]) == -1) // y belongs before name in name[k]
...
Binary search searches for a name in our name array
by taking the name, comparing it to the middle name in our array
and then sees if it's bigger or smaller than the middle, and then disgards
the other half of the array accordingly and does the check again
until it finds the correct name and returns the position in the list.
...
{	int bsrch;
	...
	for (int i = 0; i < NMAX; i++)
		lnam[i] = tolower(xname[i]); // make the name we're searching for lowercase
	while (low <= high)
	{	mid = (low + high) / 2; // calcute mid
		if (strcmp(lnam, name[mid]) == 0) // we found lnam(xname)
			return mid; // found it, return position
		else if (strcmp(lnam, name[mid]) == -1) // lnam is smaller than the middle
			high = mid - 1; // get rid of larger half
		else
			low = mid + 1; // get rid of lower half
	}
	return -1; // we didn't find lnam
}
