...
bool ChooseSort(char, int, char[][NMAX], int[], bool&);
   ...
   int location; // This is the subscript in both the WKnam and WKwght arrays where the name is found. 
	...
	system("pause");
		...
		{   system("cls");
			...
			{	cout << xnam << " was found at position " << location << " and their body weight is " << WKwght[location] << ".\n\n\n";
				system("pause");
			}
			else // Did not successfully find person
			{	cout << xnam << " was not found.\n\n";
			    system("pause");
			}
...
bool ChooseSort(char ch, int size, char WKnam[][NMAX], int WKwght[], bool& DoneSort)
{
	...
	{   case '1': 
			...
			OutList(size, WKnam, WKwght, "SORTED ARRAY DATA", "SORTED NAMES", "SORTED WEIGHTS"); // Outputting the sorted arrays
...
void SelSort(int lsize, char name[][NMAX], int weight[])
{
	...
	i = lsize - 1;
	do
	{   strcpy(big, name[0]); // Initially making the first subscript / name into the biggest spot
		weightQ = weight[0]; // Initially making the first weight the biggest
		what = 0;  // Initializing what to 0
		j = 1; // Intializing j to one
		do
